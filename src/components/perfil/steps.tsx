...
// Rediseño visual (docs/pm/design-refresh): fork de src/components/profile/steps.tsx, re-tokenizado
// para las pantallas rediseñadas — usado solo por src/app/perfil/page.tsx. Hermano: ./ui.tsx. Ver ese
// archivo y tech.md § Spec feedback para el porqué del fork. `MEAL_OPTIONS` usa el nuevo mapa de
// componentes MEAL_TYPE_ICON_COMPONENTS (en vez de interpolar el emoji de MEAL_TYPE_ICONS como string,
// que es lo que sigue haciendo el original para no romper el selector de comidas de Onboarding).
    ...
    <div className="flex flex-col gap-3">
      {(["calculated", "prescribed"] as const).map((s) => (
        <button
          ...
          className="text-left rounded-xl border border-[var(--color-border)] p-4 hover:border-[var(--color-accent)]"
        >
          <span className="block font-semibold text-[var(--color-text)]">{SOURCE_LABELS[s]}</span>
          <span id={`source-${s}-hint`} className="block text-sm text-[var(--color-text-muted)] mt-1">
            {hints[s]}
          </span>
        </button>
      ))}
...
export function SuggestedTargets({
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <details open className="rounded-xl bg-[var(--color-surface-2)] p-4 text-sm">
        ...
        <p className="mt-2 text-xs text-[var(--color-text-muted)]">Es una estimación y no sustituye a un profesional.</p>
      </details>
    </div>
  );
}
...
const MEAL_OPTIONS: Option<MealType>[] = MEAL_TYPES.map((m) => {
...
export function MealSlotPicker({ value, onChange }: { value: MealType[]; onChange: (m: MealType[]) => void }) {
  return (
    <div className="flex flex-col gap-2">
      ...
      />
      {value.length === 0 && <p className="text-xs text-[var(--color-expired)]">Elige al menos una comida.</p>}
    </div>
  );
}
...
export function AllergyDietDislikes({ value, onChange }: { value: Preferences; onChange: (p: Preferences) => void }) {
  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="flex flex-col gap-3 rounded-xl border p-4" style={{ borderColor: "color-mix(in oklab, var(--color-expired) 40%, var(--color-border))" }}>
        <MultiChoice
      ...
      />
      <p className="text-xs text-[var(--color-text-muted)] -mt-3">Intentaremos evitarlo, pero no es una regla.</p>
    </div>
  );
}
