...
## Where we are
Tasks **1–6 of 14 are done**, one commit each on the branch.

| Suite | Status |
|---|---|
| Unit (`npm test`) | 137 pass, **7 fail**: all in `tests/unit/recipes-route.test.ts`, waiting on task 7 |
| E2E (`npm run test:e2e`) | 6 pass (smoke, migration, meals), **19 fail**: onboarding, Perfil and recipes UI, tasks 8–12 |
...
- The route prompt lists allergies and diet in a single line.
...
7. **Route** `src/app/api/recipes/route.ts`:
   - Separate prompt blocks, divided by blank lines: ALERGIAS "nunca uses" with family terms, DIETA, NO LE GUSTA as a preference.
   - Protein is `proteinGoal`, the midpoint.
   - Filter the returned recipes with `recipeViolations`.
   - Return `{ recipes, droppedCount }` and read `profile.allergies` defensively.
   - Recipes UI: show "Ninguna receta era segura para tus alergias. Prueba de nuevo." when the list is empty and `droppedCount > 0`.
   - Strip `body` before the POST, since the prompt doesn't use it.
   - If you extract a prompt builder, put it in `src/lib/`. `route.ts` may only export HTTP handlers.
