...
- La frase sr-only de cada celda solo tiene ejemplos para Grasas y Proteínas; los e2e aceptan la unidad tras el objetivo como opcional ("Calorías 2030 de 2000( kcal), dentro"). Si se quiere fijar una forma, ajustar `macros-plan.spec.ts › R5`. (tests)
- R5 del spec dice "icono con nombre accesible"; la tech lo resuelve con icono `aria-hidden` + frase sr-only, y el e2e de R5 lo fija con un snapshot del árbol accesible (sin `img` dentro de la lista). (tests)
...
- Correr vitest en Windows reescribe `tests/unit/__snapshots__/shopping-parse.test.ts.snap` con finales de línea LF (diff vacío salvo EOL): falta un `.gitattributes` (`*.snap text eol=lf`) o similar. Preexistente. (tests)
- `playwright.config.ts` fija el puerto 3000 y reutiliza el servidor que haya: con varios worktrees los e2e de uno pueden correr contra el código de otro. Permitir `PORT` por variable de entorno. (tests)
